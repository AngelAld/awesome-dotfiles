...
keys.desktopbuttons = gears.table.join(
	awful.button({}, 1, function()
		...
		naughty.notify({
			...
			text = "Esta es una notificacion que deje para debugear",
		})
	...
	end), -- Middle button - Toggle dashboard
	...
	awful.button({}, 8, function()
		helpers.volume_control(-5)
	end) -- Side buttons - Minimize and restore minimized client
	-- awful.button({ }, 8, function()
...
keys.globalkeys = gears.table.join( -- Focus client by direction (hjkl keys)
	...
	awful.key({ superkey, shiftkey }, "Return", function()
		...
		group = "launcher",
	}), -- New shortcut to execute bash script
	awful.key({ altkey }, "space", function()
		awful.spawn.with_shell("~/.scripts/keyboard/layout")
	end, {
		description = "execute layout script",
		group = "custom",
	}), -- shorcut to lock screen
	awful.key({ altkey }, "l", function()
		awful.spawn("betterlockscreen -l blur")
	end, {
		description = "lock screen",
		group = "custom",
	}), -- a shortcut to open code
	awful.key({ superkey, shiftkey }, "e", function()
		awful.spawn("code")
	end, {
		description = "open code",
		group = "custom",
	}), -- a shortcut to open chrome
	...
	awful.key({}, "XF86AudioNext", function()
		awful.spawn("playerctl --player %any,chromium next")
	end, {
		description = "Next",
		group = "multimedia",
	}),
	awful.key({}, "XF86AudioPrev", function()
		awful.spawn("playerctl --player %any,chromium previous")
	end, {
		description = "Previous",
		group = "multimedia",
	}),
	awful.key({}, "XF86AudioPlay", function()
		awful.spawn("playerctl --player %any,chromium play-pause")
	end, {
